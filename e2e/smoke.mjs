...
try {
  ...
  await desktop.getByRole('heading', { name: /Alex M\. \(synthetic\) · Learning timeline/ }).waitFor();
  await desktop.getByText('Visual choices').first().waitFor();
  ...
  await desktop.getByRole('button', { name: /Student activity/ }).click();
  await desktop.getByTestId('reset-demo').click();
  await desktop.getByText('Attempt in progress').waitFor();

  const mobile = await browser.newPage({ viewport: { width: 390, height: 844 }, deviceScaleFactor: 1 });
  await mobile.goto(baseUrl, { waitUntil: 'networkidle' });
  await mobile.getByTestId('support-break').click();
  await mobile.getByTestId('support-response').waitFor();
  ...
  console.log('E2E passed: local-store persistence/reset, support fallback, qualifying evidence confirmation, locked future phase, and mobile layout.');
} finally {
  await browser?.close();
  server.kill('SIGTERM');
}
